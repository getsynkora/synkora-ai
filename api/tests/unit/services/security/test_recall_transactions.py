...
def signed_request():
    ...
    signature = base64.b64encode(hmac.new(b'secret',f'msg-transaction.{timestamp}.'.encode()+payload,hashlib.sha256).digest()).decode()
    async def receive():
        return {'type':'http.request','body':payload,'more_body':False}
    return Request({'type':'http','method':'POST','path':'/','query_string':b'','headers':[(b'webhook-id',b'msg-transaction'),(b'webhook-timestamp',timestamp.encode()),(b'webhook-signature',('v1,'+signature).encode())]},receive)
