...
def verify_capability(token: str, secret: str, tenant: str, workspace: str) -> bool:
    try:
        ...
        now = int(time.time())
        # Allow 5-second grace period on the lower bound for clock skew
        # between the API server (issuer) and the sandbox service (verifier).
        return (
