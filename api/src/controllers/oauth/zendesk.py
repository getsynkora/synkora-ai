"""
Zendesk OAuth Controller.

Handles Zendesk OAuth 2.0 authorization and callback.
Requires 'subdomain' in the OAuthApp.config field.
"""
...
async def zendesk_authorize(
    ...
    """Initiate Zendesk OAuth authorization."""
    try:
        ...
        if oauth_app.auth_method == "api_token":
            return _safe_success_redirect(redirect_url, "/oauth-apps", base_url, "zendesk", method="api_token")
        ...
        try:
            ...
            logger.error("Failed to decrypt Zendesk client secret: %s", e)
        ...
        logger.info("Initiating Zendesk OAuth for app %s (user_level=%s)", oauth_app_id, user_level)
        return RedirectResponse(url=auth_url)

    except HTTPException:
        raise
    except Exception as e:
        logger.error("Zendesk OAuth authorization error: %s", e, exc_info=True)
...
async def zendesk_callback(
    ...
    """Handle Zendesk OAuth callback and store token."""
    try:
        ...
        subdomain = (oauth_app.config or {}).get("subdomain", "").strip()
        if not subdomain:
            raise HTTPException(status_code=400, detail="Zendesk subdomain not configured")
        ...
        if user_level and account_id:
            ...
            logger.info("Zendesk OAuth successful (user-level) for app %s, user %s", oauth_app_id, user_email)
        else:
            ...
            logger.info("Zendesk OAuth successful (app-level) for app %s, user %s", oauth_app_id, user_email)
        ...
        logger.error("Zendesk OAuth callback error: %s", e, exc_info=True)
        ...
        if oauth_app_id:
            ...
            if oauth_app:
                base_url = await get_app_base_url(db, oauth_app.tenant_id)
        return _safe_error_redirect(redirect_url, "/oauth-apps", base_url, "zendesk", e)
