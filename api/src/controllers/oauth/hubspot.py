...
Handles HubSpot OAuth 2.0 authorization and callback.
Also supports Private App tokens (api_token auth method — no OAuth flow needed).
"""
...
async def hubspot_authorize(
    ...
    try:
        ...
        # Private App tokens skip the OAuth dance
        if oauth_app.auth_method == "api_token":
            return _safe_success_redirect(redirect_url, "/oauth-apps", base_url, "hubspot", method="api_token")

        client_id = oauth_app.client_id
        try:
            client_secret = decrypt_value(oauth_app.client_secret)
        except Exception as e:
            logger.error("Failed to decrypt HubSpot client secret: %s", e)
        ...
        logger.info("Initiating HubSpot OAuth for app %s (user_level=%s)", oauth_app_id, user_level)
...
async def hubspot_callback(
    ...
    """Handle HubSpot OAuth callback and store token."""
    try:
        ...
        if user_level and account_id:
            ...
            logger.info("HubSpot OAuth successful (user-level) for app %s, user %s", oauth_app_id, user_email)
        else:
            ...
            logger.info("HubSpot OAuth successful (app-level) for app %s, user %s", oauth_app_id, user_email)
        ...
        logger.error("HubSpot OAuth callback error: %s", e, exc_info=True)
        ...
        if oauth_app_id:
            ...
            if oauth_app:
                base_url = await get_app_base_url(db, oauth_app.tenant_id)
        return _safe_error_redirect(redirect_url, "/oauth-apps", base_url, "hubspot", e)
