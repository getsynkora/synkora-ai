"""
Intercom OAuth Controller.

Handles Intercom OAuth 2.0 authorization and callback.
Also supports Personal Access Tokens (api_token auth method — no OAuth flow needed).
"""
...
from ...services.oauth.intercom_oauth import IntercomOAuth
...
async def intercom_authorize(
    ...
    """Initiate Intercom OAuth authorization."""
    try:
        ...
        # Personal Access Token — skip OAuth dance
        if oauth_app.auth_method == "api_token":
            return _safe_success_redirect(redirect_url, "/oauth-apps", base_url, "intercom", method="api_token")

        client_id = oauth_app.client_id
        try:
            client_secret = decrypt_value(oauth_app.client_secret)
        except Exception as e:
            logger.error("Failed to decrypt Intercom client secret: %s", e)
        ...
        logger.info("Initiating Intercom OAuth for app %s (user_level=%s)", oauth_app_id, user_level)
        return RedirectResponse(url=auth_url)

    except HTTPException:
        raise
    except Exception as e:
        logger.error("Intercom OAuth authorization error: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/intercom/callback")
async def intercom_callback(
    code: str = Query(...),
    state: str = Query(...),
    db: AsyncSession = Depends(get_async_db),
):
    """Handle Intercom OAuth callback and store token."""
    try:
        ...
        if user_level and account_id:
            ...
            logger.info("Intercom OAuth successful (user-level) for app %s, user %s", oauth_app_id, user_email)
        else:
            oauth_app.access_token = encrypt_value(access_token)
            oauth_app.token_expires_at = None
            logger.info("Intercom OAuth successful (app-level) for app %s, user %s", oauth_app_id, user_email)
        ...
        logger.error("Intercom OAuth callback error: %s", e, exc_info=True)
        ...
        if oauth_app_id:
            ...
            if oauth_app:
                base_url = await get_app_base_url(db, oauth_app.tenant_id)
        return _safe_error_redirect(redirect_url, "/oauth-apps", base_url, "intercom", e)
